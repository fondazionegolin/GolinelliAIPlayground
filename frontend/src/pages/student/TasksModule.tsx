...
function TaskCard({ task, onClick }: { task: TaskData; onClick: () => void; accentColor: string }) {
  ...
  const tileColor = useMemo(() => {
    if (isCompleted) return 'bg-emerald-500 hover:bg-emerald-600'
    switch (task.task_type) {
      case 'quiz': return 'bg-rose-500 hover:bg-rose-600'
      case 'lesson': return 'bg-blue-600 hover:bg-blue-700'
      case 'presentation': return 'bg-indigo-500 hover:bg-indigo-600'
      case 'exercise': return 'bg-amber-500 hover:bg-amber-600'
  ...
  return (
    <motion.div
      whileHover={{ scale: 1.02, boxShadow: '0 8px 20px rgb(0 0 0 / 0.15)' }}
      whileTap={{ scale: 0.97 }}
      onClick={onClick}
      className={`aspect-square relative cursor-pointer rounded-2xl text-white shadow-sm transition-all flex flex-col items-center justify-center p-4 ${tileColor}`}
    >
      {isCompleted && (
        <div className="absolute top-2.5 right-2.5 bg-white/25 rounded-full p-0.5">
      ...
      <span className="text-xs font-bold leading-tight text-center line-clamp-2 px-1">{task.title}</span>
      {task.due_at && !isCompleted && (
        <div className="flex items-center gap-1 mt-1.5">
          <Clock className="h-3 w-3 opacity-70" />
          <span className="text-[10px] opacity-70">{new Date(task.due_at).toLocaleDateString('it-IT')}</span>
        </div>
      )}
    </motion.div>
  )
